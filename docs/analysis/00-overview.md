...
| # | Question | Blocks | Best way to close | Doc |
|---|---|---|---|---|
| O1 | Tick period and unit of opcode-3000 increments (2.1–2.2 units/tick at 50 mm/s, ≈8 at rapid; 250 µs × 258 p/mm predicts 3.2); frame-id semantics (restarts without `NC Start`, reaches 1931 in 10 s); what `MaxItemPerFrame=60` limits vs 99 observed | job streaming | capture one 100 mm move at 100 mm/s and count items/frames/time | 08 §9.1, §9.11, 04 §10 |
| O2 | Bit meaning of status/alarm words (1000+4…+8), axis status (2000+0), exception codes 2/3 precise semantics | any UI, safety | capture idle, e-stop, limit hit, jog-while-moving | 04 §10, 06 §9.1 |
| O3 | Jog/home argument units and index-vs-mask (home 2nd word 0x1F seen; sub-cmd 3 axis 0/1/4); home `[1,axis,2,2000,20000]` speeds vs configured 80/20 mm/s | milestone 1 | capture each jog key/home with known settings | 04 §3.6, 08 §4.5 |
| O4 | Opcodes 3001/3002/103/109/2001/118 semantics; which DOs bits 2 and 8 are (see §10: LIKELY DO3 high-air, DO9 CO2 enable); which output the 0x65 `[9999,2,1,x]` sets | streaming | capture with different gas/laser settings | 08 §9.2 |
| O5 | Block 10000/11000/13000/13200 contents; why 10000 stalls > 3.5 s; poll cadence | driver design | capture + register naming | 04 §3.5, 08 §4.3 |
| O6 | Licence registers / data area / RTC commands; does firmware refuse motion when unlicensed (evidence says PC-side) | port viability | capture start-up after `Active OK`; test with expired licence | 07 §14, 08 §9.6 |
| O7 | Axis slots 3/4 (Z vs W), which slot the lifting table jogs; Y dual-drive (`DoubleDriver=1` vs `DoubleDevice=0`) | homing/lifting | UI Advanced page or capture which 2000-block slot moves | 01 §7 |
| O8 | Limit-switch polarity per pair (DI5/6, DI7/8, …) | homing safety | bench: press each switch, read DI word | 01 §1 |
| O9 | ZF (height follower) protocol on port 999; whether CO2 jobs command it; FTC register semantics (`ZFReadOnly*`) | fibre cutting, edge seek | capture port 999 during a fibre job / calibration | 04 §10, 02 §7 |
| O10 | Fibre laser control when fibre is selected (IO/DA vs 10.1.1.170 Ethernet); DA1 scaling of "peak current %" | fibre cutting | capture with fibre selected | 02 §7.9, 04 §4.3 |
| O11 | `.chf` crafts scalars (`int58`, `double170/188`, PWM pair meaning, lead enum, compensate 2 vs 3, cool-pos unit, trailer bool/double, text/spline flags, type-12 links) | file round-trip | diff `autosave.chf` after UI actions under Wine | 03 §12–13 |
| O12 | Exact UI → planner block mapping (P4/P5/P8/P9; whether `AccTime` is /1000 in MainApp); which ratio list feeds split-and-stop; which flows use node-builder B | planner fidelity | disassemble MainApp caller of `CCADModule+0x…` wrapper, or black-box tests under Wine | 05 §13 |
| O13 | `ManuType` codes for "advanced fixed height"; folding of the two combos | layer editor | Wine test + diff `BkLayerPara.xml` | 02 §7 |
| O14 | `.enc`/`.aut` binary layout; `NormalExit` write path; `AutosaveParam` event; `TotalReport` row trigger | interchange, resume | Wine tests | 02 §7, 08 §9 |
| O15 | Alarm-id ↔ bit mapping (`gp1–32` one word?); how numbered ids are assembled; duplicate-id policy in LangModule | alarm panel | disassembly of the alarm decoder | 06 §9 |
| O16 | PHBX pendant HID report layout; hand-wheel report layout | pendant support | USB capture | 07 §14 |
| O17 | `.mcf` cipher, CPU type | firmware update from Linux | not needed if Windows tool keeps that job | 04 §10 |
| O18 | Which value sits at `[param+0x4356]` that forces TCP; does the card also accept TCP | transport | try TCP once in a capture session | 04 §10 |
