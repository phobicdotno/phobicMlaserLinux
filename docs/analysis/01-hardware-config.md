...
| `File/autosave.chf` | 10 431 B | "scFlie" container (job file) | auto-saved copy of the last job (not analysed here; see 03-chf-format) — *added by verifier* |
| `File/BkLayerPara.xml` | 44 148 B | ASCII XML | layer/process parameters (out of scope, see 02-layer-params) — *added by verifier* |
| `File/PM/` | empty dir | — | target of `\File\PM\p0.pmf` in the load sequence (§0.3); no file shipped — *added by verifier* |
| `File/Temp/AutosaveParam1.ini`, `2.ini`, `ManuContour.dat`, `tempIsBreak.ini`, `tempGraph.chf` | 40/40/22/19/500 B | "scFlie" containers | snapshot of an earlier job (Apr 23 2025). These five are the **only** files in `File/Temp/` (verifier: `ls`); `tempLayer.xml`/`tempManu.ini` exist only as strings in MainApp.exe |
...
I parsed 1 001 descriptors out of the disassembly (`objdump -d -M intel MainApp.exe`) and joined them (verifier re-count: the pattern `mov ds:X,eax` immediately followed by `mov DWORD PTR ds:X+4,<type>` occurs 1 004 times between `0x458412` and `0x7ab430`, three of which are false positives with impossible type codes — so ≈1 001 is right; type-code histogram: 1×216, 2×289, 3×29, 4×140, 5×198, 6×46, 7×44, 8×9, 9×5, 10×3, 11×3, 12×14, 13×3, 14×2) with the two XML files and `Lang/lang.txt`. That join is the basis of the inventory tables in §1. **Every XML attribute present in this machine's `BkHardPara.xml` and `BkManuPara.xml` has a descriptor in the executable (777 keys checked, 0 unknown)**; the descriptors that have no counterpart in these two files belong to `BkLayerPara.xml` (`GP.*` layer/process parameters, out of scope here), to `GRP.Zoom*`, or to test dialogs (`LT.*`, `ST.*`).
...
INFERENCE `[likely]` (downgraded by verifier from `[confirmed]`): the "Bk" prefix means *backup copy*: MainApp keeps the live parameters in memory, writes them to `BkHardPara.xml` / `BkManuPara.xml`, and keeps `SecondBkManuPara.xml` as a second-level backup of the machining parameters. **Verifier caveat:** the primary files named first in the load sequence (`\SystemPara.xml`, `\HardPara.xml`, `\ManuPara.xml`, `\LayerPara.xml`, UTF-16 strings at `.rdata` VA `0x7e5a5c` etc.) do **not** exist anywhere in the shipped package (`find SRC -iname '*Para*.xml'` returns only the four `File/Bk*`/`SecondBk*` files), so either they live outside the package directory or the "primary" path is a legacy that is never written; which file the running program actually treats as authoritative could not be established from static evidence alone (see Open questions). on read failure it falls back to the backup and, failing that, to the second backup (and moves the broken file to `ErrorManuPara.xml` / `ErrorBkManuPara.xml`). `hp51`: "硬件配置参数保存失败，请重新保存！" (hardware configuration save failed, please save again). See §5.
...
> **Verifier caveat on limit-input naming (affects §2.2, §2.3, §2.8):** the descriptor for `MAC*.NegativeLimitInput` carries the label id `EtherAxisInfos_8` = `限位.正限位输入端口` ("**Positive** limit input port") and `MAC*.ForwardLimitInput` carries `EtherAxisInfos_9` = `限位.负限位输入端口` ("**Negative** limit input port"); likewise `NegativeType` → `EtherAxisInfos_1` (正限位开关逻辑, positive-limit switch logic) and `ForwardType` → `EtherAxisInfos_2` (负限位开关逻辑). EVIDENCE: descriptor initialisers at `0x790171` (`push 0x87ba00` = "MAC.NegativeLimitInput", then `push 0x87b9dc` = "EtherAxisInfos_8", value `g+0xbb40`, type 7) and `0x7902e7` (`push 0x87b8f8` = "MAC.ForwardLimitInput", `push 0x87b8d4` = "EtherAxisInfos_9", value `g+0xbb44`, type 7); `lang.txt` lines `EtherAxisInfos_8#限位.正限位输入端口#Limit. Positive limit input port` and `EtherAxisInfos_9#限位.负限位输入端口#Limit. Negative limit input port`. So **the XML attribute name and the label the technician saw in the UI contradict each other**: what the file calls `NegativeLimitInput=6` was configured on screen as the *positive* limit port. Which of the two the firmware honours is not determinable from MainApp alone; every "DI5 = +, DI6 = −" statement below is therefore `[guess]` (attribute-name reading) with the UI-label reading (DI6 = +, DI5 = −) equally possible.
...
2. `PMachineAxisConfig` + `PMachineAxisConfig_0..4` (`MAC`, `MAC_1..MAC_4`) — the newer per-slot model whose labels are `EtherAxisInfos_*` / `GoHomeAdv_*` / `AllAxisInfo_*` ("轴基本参数" basic axis parameters). Five slots. INFERENCE (verifier-corrected): slots **0 = X, 1 = Y1 (master), 2 = Y2 (dual-drive slave)** are `[confirmed]`; the roles of slots **3 and 4 (Z vs W) are `[guess]`**. EVIDENCE found by the verifier: the settings object's constructor calls `0x40b290` with `ecx = g+0xba58`, which initialises a small *axis-index list*: `[+0]=2` (count of active axes), `[+4]=0`, `[+8]=1`, `[+0xc]=2`, `[+0x10]=3`, `[+0x14]=4` (i.e. `g+0xba5c=0`, `g+0xba60=1`, `g+0xba64=2`, `g+0xba68=3`, `g+0xba6c=4`), `[+0x48]=1` (`g+0xbaa0`, Y master slot), `[+0x4c]=2` (`g+0xbaa4`, Y slave slot), `[+0x58]=1000`. These fields are never written anywhere else in MainApp (only copied whole at `0x40b702`/`0x4e9c39`, read at `0x4893c0` which builds the bit-mask `1<<index` over the first `count` entries, and compared at `0x5ef462`). The dual-drive path `0x4b06a5–0x4b07f8` tests `MAC.DoubleDevice` (`g+0xba9c`) and copies the per-slot fields of slot `g+0xba60` (Y1) into slot `g+0xbaa4` (=2, Y2) using the `0xa0`-byte slot stride — which pins slot 2 as the Y2 slave. The `softPara.ini` writer (§4) uses `g+0xba64=2` for the key `ZAxis` and `g+0xba68=3` for `WAxis`, so the INI key names do **not** coincide with the `A250516_0..4` naming ("X轴/Y1轴/Y2轴/Z轴/W轴") — under that list slot 3 would be Z and slot 4 W, whereas the INI writer calls slot 2 "Z" and slot 3 "W". The card's own register names know only four pulse axes `X/Y1/Y2/W` (`RegName9..12`, `RegName63..70`), which fits slots 0..3 with **slot 3 = W**, leaving `MAC_4` unexplained. The original analyst's statement that "slot 4 homes in the negative direction" was wrong: `MAC_4.GoOriginalDirection=1` = 正向 positive (the table below was already right). The lifting-platform handler (`LPF.LiftingPlatformType`, value `g+0x4d68`, tested at `0x46c7d4`, `0x57e2a6`, `0x57e5ec`) does not read any of the `g+0xba5c..0xba6c` index fields, so which slot the W-axis jog drives could not be traced statically — **open question** (see §7).

| Slot / role | `SoftLimitMaxLen` (mm) | `WritePluse` (pulses/rev) | `SpeedRatio` = lead (mm/rev) | derived pulses/mm | `AxisReverse` | Home dir (`GoOriginalDirection`) | `ReturnLength` (mm, home back-off) | `FastSpeed`/`SecondSpeed` (mm/s) | Limit inputs `NegativeLimitInput`/`ForwardLimitInput` (DI#; **UI labels call these Positive/Negative respectively — see §1 caveat**) | `Acceleration` (mm/s²) / `AccelerationTime` (ms) |
|---|---|---|---|---|---|---|---|---|---|---|
...
| `MAC_3` = Z *or* W (`[guess]`; the exe's `WAxis` INI accessor and the card register names point to slot 3 = W) | 1000 (default) | 10000 (default) | 5 | 2000 | 0 | 0 | 10 | 30 / 20 | DI9 / DI10 | 20000 / 125 |
| `MAC_4` = W *or* unused 5th slot (`[guess]`; original analyst: lifting platform) | 1000 (default) | 2000 | 10 | 200 | 1 | **1 = 正向 positive** | 5 | 30 / 20 | DI2 / DI3 | 20000 / 125 |

EVIDENCE for the meaning of the columns: descriptor labels `EtherAxisInfos_4` "轴基本参数.最大行程" (max stroke), `A241021_1` "轴基本参数.每转脉冲数" (pulses per revolution, English "Encoder resolution"), `EtherAxisInfos_10` "轴基本参数.导程(mm)" (lead, mm), `EtherAxisInfos_5` "运行反向" (run reversed), `EtherAxisInfos_6` "编码器反向", `EtherAxisInfos_7` "回原方向", `GoHomeAdv_3/4/5` "粗定位速度/精定位速度/返回距离" (coarse/fine homing speed, return distance), `EtherAxisInfos_8/9/11` "**正**限位/**负**限位/原点输入端口" (**positive**/negative/origin input port — verifier correction: `_8` is 正 positive and is the label of `NegativeLimitInput`; `_9` is 负 negative and is the label of `ForwardLimitInput`), `EtherAxisInfos_1/2/3` "正限位/负限位开关逻辑, 原点输入类型" (switch logic NO/NC; `_1` labels `NegativeType`, `_2` labels `ForwardType`), `EtherAxisInfos_23` "抱闸.输出端口" (brake output), `EtherAxisInfos_24` "作为旋转轴" (is rotary axis), `EtherAxisInfos_27/28` "工作参数.加速度/加速时间", `GoHomeAdv_0/1/2` "使用Z相信号/二次回原/采样信号" (use Z-pulse / second homing / sample signal = origin switch vs limit switch).
...
* INFERENCE `[likely]` for the direction, `[guess]` for the port: X and Y home in the negative direction (`GoOriginalDirection=0` = 负向) onto their negative limit switch, then back off 22/15 mm; machine origin is therefore the negative-limit corner. **Which DI is the negative switch is ambiguous** (§1 caveat): by attribute name it is `NegativeLimitInput` = DI6 (X) / DI8 (Y); by the UI label under which the value was entered (`EtherAxisInfos_9` 负限位输入端口 = `ForwardLimitInput`) it is DI5 (X) / DI7 (Y). Bench-check before relying on either. `SOP.IsStartNeedGoOrigin=1` (`pd530` "开机是否提示回原" prompt to home at start-up) and `mp119` "硬件连接成功，是否对系统进行回原？".
...
| `CO2LaserControlType` | **2** | `pd640`, `A250522_1..3`: 0 None, 1 **24V PWM**, 2 **5V PWM**, 3 DA → **5 V PWM** (verifier re-checked descriptor at `0x794c31–0x794cb4`: label `pd259`, value `g+0x4940`, type 4, default `"2"`, min 0, max `3.0` (`0x7e32f0`), options `0xa48fc8`×4 built at `0x780442–0x78047e`) |
| `CO2LaserDAPort` | 1 | DA1 |
| `CO2LaserDAType` | 1 | 0–5 V |
| `CO2DORemoteStart` | 0 | none |
| `CO2DOLaserGate` | 0 | none |
| `CO2DOLaser` | **9** | descriptor label `pd264` "IO.激光输出" (IO.Laser Emission; verifier correction — `A241104_3` "CO2 激光" is a separate string, presumably the I/O-monitor name) laser-enable output → DO9 |
...
| `iProportionalOpenSleep` | 50 ms | 杂项.比例阀开启前延时 (label id `A241224_0`; **note** `lang.txt` defines `A241224_0` twice — once as `记忆零点`/"Memory zero", once as `杂项.比例阀开启前延时`/"Misc.Proportional valve opening delay" — which is why §1 shows "记忆零点"; the file has 5 duplicated ids in total: `A241224_0`, `A250616_0`, `gp100`, `pd1001` and one blank) | delay before opening proportional valve |
...
| DI2 / DI3 | slot-4 axis limits (`NegativeLimitInput` / `ForwardLimitInput`; polarity ambiguous, see §1 caveat; slot-4 role Z/W is `[guess]`) | NO | `MAC_4.NegativeLimitInput=2`, `ForwardLimitInput=3` |
| DI4 | custom alarm "Door" | see below | `DI.AlarmDIStr="Door#4#-1#0"` |
| DI5 / DI6 | X limits: by attribute name DI5 = `ForwardLimitInput` (+), DI6 = `NegativeLimitInput` (−, home switch); by UI label the reverse (DI6 = 正限位 +, DI5 = 负限位 −) — `[guess]` either way | NO | `MAC.ForwardLimitInput=5`, `NegativeLimitInput=6` |
| DI7 / DI8 | Y limits: same ambiguity (DI7 = `ForwardLimitInput`, DI8 = `NegativeLimitInput`) | NO | `MAC_1.ForwardLimitInput=7`, `NegativeLimitInput=8` |
| DI9 / DI10 | slot-3 axis limits (`NegativeLimitInput` / `ForwardLimitInput`; slot-3 role Z/W `[guess]`) | NO | `MAC_3.NegativeLimitInput=9`, `ForwardLimitInput=10` |
...
`SOP.RemoteType=3` although the descriptor's option list has only three entries (`pd178/pd179/pd180` = "SC板载" SC MCC (card-attached receiver), "SC电脑" SC PC (USB), "CypCut") and `max=1`. Code at `0x498ed6` (`cmp [g+0x4028],3; jne`) calls `0x586e70`, which `LoadLibraryW(L"PHBX.dll")` and calls its `Xinit`/`XOpen` exports; a 4-way `switch` on the same field at `0x597cfa` has cases 0,1 → `0x599a80`, 2 → `0x59a010`, 3 → `0x599380`. INFERENCE `[confirmed]`: **RemoteType 3 = XHC "PHB02/PHBX" wireless pendant** (PHBX.dll version info: `CompanyName "chengdu XHC Tec."`, `FileDescription "PHB02"`, `ProductName "XHC PHBX"`, exports `Xinit XOpen XClose XGetInput XSendOutput XGetChannel XGetDevID XGetDevRssi XGetDevTxstate XGetFID XGetVersion XWritDev XWritFid`, imports `HidD_GetAttributes/HidD_GetFeature/HidD_SetFeature/HidP_GetCaps`, `SetupDi*`). **Verifier downgrade to `[guess]`** of the original claim that the USB-HID device VID 3689 / PID 8762 ("USBKey") on this PC is the PHBX receiver: neither PHBX.dll nor MainApp.exe contains 3689/8762 (decimal or hex, as 16- or 32-bit constants or as text). MainApp's *own* HID matching code (`0x59b0ba–0x59b171`, selected when `RemoteType` is 1 or 2) compares against the UTF-16 strings `"2016"` (VID), `"6125"`/`"2012"` (PID) with `"VID_"`/`"PID_"` (`.rdata 0x843e20–0x843e68`) — i.e. the "SC PC"/"CypCut" USB receivers use VID 2016. The name "USBKey" and the presence of a licence check (`dogActiveReslut1..11`: "加密-未找到硬件/硬件ID不匹配/厂商ID不匹配/板卡时钟过慢" = encryption: no hardware / hardware-ID mismatch / vendor-ID mismatch / board clock too slow; `ipAdd.ini` `CheckUserID=109`) make a licence dongle at least as plausible, although those messages mention the *board* (板卡) clock, so the licence may also be card-bound. Unresolved (see §7). `A250613_0..5` messages ("手柄模块加载成功/断开/电量低/信号差/休眠/唤醒" = pendant module loaded / disconnected / low battery / poor signal / sleep / wake). `SOP.JoystickID1..5` (179705051, −684904636, 434577794, 1601441, 242474605) are the pairing codes (`pd293..295` "手柄.配对码1..3"; `RegName40..46/90..96` hold the wireless-handle address bytes on the card for the "SC MCC" type). `A241105_0` "Wifi" is another option present in the strings but not in this list.
...
EVIDENCE:
* Written at exit by the function `0x45cf80` (body `0x45d015–0x45d1ec`), called from the exit routine that logs `--- Exit Sys ---` (`push 0x7da348` at `0x459886`, call at `0x459ab4`): for each of the four index fields `g+0xba5c/0xba60/0xba64/0xba68` (= slots 0,1,2,3 — see §2.2 verifier note; **not** "X,Y,Z,W = 0,1,3,4") it calls the HAL virtual method at vtable offset `0x1a0` with arguments `(3, slot*10 + 2)`, converts the returned 32-bit integer to text (`0x44a7e0`) and `WritePrivateProfileStringW(L"SC2000", key, text, "\File\softPara.ini")` with key `XAxis` (`0x7da7a8`) ← slot `g+0xba5c`, `YAxis` (`0x7da7c4`) ← `g+0xba60`, `ZAxis` (`0x7da7e0`) ← `g+0xba64` (= slot 2, the Y2/slave slot), `WAxis` (`0x7da7fc`) ← `g+0xba68` (= slot 3). `NormalExit` is written as `"1"` in the same exit routine (`0x459a9e`: `WritePrivateProfileStringW("SC2000","NormalExit","1")`, string `0x7da3c8`) and by a second exit path at `0x48f32e` (string `0x7dd6a8` = `"1"`); it is written as `"0"` at `0x56835b` (string `0x841dd0`) in the start-up routine. **Verifier correction:** the analyst's `0x567c3e` is not a write — it is `push 1` = the *default* argument of `GetPrivateProfileIntW("SC2000","NormalExit",1)` at `0x567c40–0x567c4a`, whose result is stored in `g+0x3ff8` and tested at `0x567c62` (`cmp [g+0x3ff8],1`).
* Read at start-up (`0x567b71`): `GetPrivateProfileIntW(L"SC2000", L"XAxis", 0)` → stored into the per-slot axis structure field `g + 0xbb54 + slot*0xa0` (the same field the run-time refresh at `0x5d95af–0x5d9602` fills from the card register `slot*10+2` via the same vcall `+0x1a0`); the reader then does the same for `YAxis` (`0x567bb6`, slot `g+0xba60`) and `ZAxis` (`0x567bfb`, slot `g+0xba64`) — **`WAxis` is written but never read back** (verifier: no reference to the `WAxis` string other than the writer). `NormalExit` read with default 1. If `NormalExit==0` the app shows `mp18` "检测到系统最后一次加工为非正常退出，是否恢复到最近的加工状态？" ("last run ended abnormally, restore the last processing state?") and `mp19/mp21`, then reloads `autosave.chf` / `ManuContour.dat` / `AutosaveParam*.ini` (see §6.3).
...
EVIDENCE: writer at `0x446c50` (only when the machining state `g+0x47cc == 2`): line 1 = global `ds:0x9495ac` (current item index), line 2 = `round(x * 1000.0)` (`fmul ds:0x7d0ec8` = 1000.0 — verifier read the double: exactly `1000.0`; then `0x4511d0` double→int), line 3 = `round(y * 1000.0)`, lines 4–5 two more integers, then `eof`. **Verifier addition — which of the two files is written:** `0x446cd4–0x446d04` takes the parity of `ds:0x9495ac` (`and edx,0x80000001`) and selects the path object at `this+0x174` for an even item index and `this+0x190` for an odd one; this matches all four shipped samples (`AutosaveParam1.ini` holds even indices 38 and 30, `AutosaveParam2.ini` odd indices 37 and 31). So the two files are not "latest / previous" but "even / odd item", which still guarantees that a crash while writing one leaves the other intact. Reader (`0x43c9a2–0x43ca9a`, two copies for Param1/Param2): `openRead`, `readInt`, `readDouble`, `readDouble`, `readInt`, `readInt`; the two doubles are then scaled by `0x7d0eb8` = **0.001** (`0x43ca47`; verifier read the constant: exactly `0.001`) into a point. Diagnostic format `"read Index:%d -- read point:(%d,%d) -- Graph Index:%d"`. `mf147`-adjacent messages: `"Graph Index file was error!"`, `"Broken point file was error!"`.

INFERENCE `[confirmed]` for fields 1–4, `[likely]` for field 5: `AutosaveParamN.ini` = break-point record *N* (two alternating slots so that a crash while writing one leaves the other intact): **[1] contour/item index in `ManuContour.dat` order, [2] X position in 0.001 mm, [3] Y position in 0.001 mm, [4] graph (glyph) index, [5] interpolation-point index inside the contour** (`"read index:%d(%d + %d = %d)"` adds an offset to it). Example: the last job stopped at contour 38, at (979.454 mm, 342.126 mm), graph 7, point 486. `tempIsBreak.ini` = `isBreak` flag `1` + a second flag (`[likely]` "break-point valid"). The `File/Temp/` set (`tempGraph.chf`, `tempIsBreak.ini`, `ManuContour.dat`, `AutosaveParam*.ini` actually present; `tempLayer.xml` and `tempManu.ini` are named in MainApp's strings but are not in the shipped folder) is the *task package* staging area: the "导入任务/导出任务" (import/export task, `A250320_*`) feature bundles exactly these files with the section tags `TASK_MANU`, `TASK_PARAM_ONE`, `TASK_PARAM_TWO`, `TASK_IS_BREAK`, `TASK_LAYER` (ASCII tags at `0x7db638…`, function `0x4686xx`).
...
1. **Axis-slot → role mapping for slots 3 and 4** (`PMachineAxisConfig_3/_4`): slots 0/1/2 = X/Y1/Y2 are now confirmed from the constructor `0x40b290` and the dual-drive copy code `0x4b06a5–0x4b07f8` (verifier). The `WAxis` INI accessor uses slot 3 and the card's register names list only X/Y1/Y2/W, which suggests slot 3 = W and slot 4 = a fifth (Z?) slot, while the UI name list `A250516_0..4` suggests slot 3 = Z, slot 4 = W. Which slot the lifting-platform Up/Down buttons (`A250410_3/4`, created at `0x453007`/`0x45311e`) actually jog was not traced. Verify on the running system (Advanced page `AllAxisInfo`) or by watching which register slot moves when the table is jogged.
1a. **Limit-switch polarity** (verifier): `MAC*.NegativeLimitInput` is labelled 正限位输入端口 (positive) in the UI and `ForwardLimitInput` 负限位 (negative). Decide from the firmware/protocol side (or bench test) whether DI5 or DI6 is X−, DI7 or DI8 is Y−.
1b. **Where are the primary parameter files?** (verifier) `\SystemPara.xml`, `\HardPara.xml`, `\ManuPara.xml`, `\LayerPara.xml` are named first in the load sequence but do not exist in the package; only the `Bk*`/`SecondBk*` copies do. Trace the directory prefix used with those strings (`0x7e5a5c` and neighbours) — it may be a legacy path or a location outside the package.
1c. **What is the USB device VID 3689 / PID 8762 "USBKey"?** (verifier) Neither MainApp.exe nor PHBX.dll references these ids; MainApp's own HID code looks for VID 2016 / PID 6125|2012. Candidates: the XHC pendant receiver (PHBX.dll matches by some other criterion), or a licence dongle (`dogActiveReslut*`).
...
3. **Units of the card position registers**: the 0.001 mm interpretation of `softPara.ini` values is consistent but not proven from the protocol (verifier: the stored field `g+0xbb54` is never read back through that offset in MainApp, so no scaling instruction exists to cite); confirm by reading `AxisRORegName_3` live (protocol analyst). Note also that the `ZAxis`/`WAxis` keys hold slots 2 and 3, not necessarily a physical Z/W.
...
* The **I/O function → port** mapping and gas-valve/proportional-valve/PWM model of §2.5, §2.7, §2.8: this is the machine's electrical contract and must be configurable identically (DO1 alarm lamp, DO2 high-N2, DO3 high-air, DO7 low-O2, DO9 CO2 laser enable, DA2 O2 proportional 0–10 V ≙ 0–10 bar, 5 V PWM for tube power, DI limit pairs 5/6 (X), 7/8 (Y), 9/10 (slot 3), 2/3 (slot 4) — **with the +/− assignment inside each pair still to be settled (§1 caveat)**, DI11 chiller NC alarm, DI4 door). A port must keep the two attribute names `NegativeLimitInput`/`ForwardLimitInput` verbatim and treat their polarity as a per-machine setting until the firmware side is understood.
...
* Axis-slot model: five `PMachineAxisConfig_n` slots with a hard-coded index list `{X=0, Y1=1, Y2(slave)=2, 3, 4}` (constructor `0x40b290`); a port should keep the slot numbering and the master/slave slot pair (1/2) exactly, and expose the slot-3/4 roles as configuration until resolved.
* Break-point files: `AutosaveParam1.ini` receives even item indices, `AutosaveParam2.ini` odd ones (§6.3); a compatible implementation must keep that parity rule so the vendor software can read the files back.
