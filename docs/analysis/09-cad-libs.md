...
| `SmartNest.dll` (946 KB, VC6, PE timestamp 2002‑10‑21) | **NestLib** by **Geometric** (Geometric Software Solutions, India) — "Nestlib Development Version", unix path `/mna/usr/home/nestlib/tbom`, date string `20/09/2002`. **[VERIFIER] The NestLib release version is *not* established**: the only version stamp, `VERSION : 3.0, DATE : 30 JUN 95`, sits next to `Part%d`/`Mirror%s` and is a 1995 stamp of an internal component, not a "NestLib 3.x" label | Proprietary (Geometric/HCL NestLib), hardware‑lock protected ("Can not find the hardware lock") | True‑shape nesting kernel (loaded dynamically by AutoNest as `smartnest.dll`) |
...
| `CircleFitDLL.dll` (70 KB, 2015‑03‑05, no CRT imports) | Small in-house **least-squares circle/arc fitting** library (`CCircleFit::mainFit`) with 4 tuning globals | Proprietary in-house | `mainFit` is called only from `Dxf2Grp.dll` (3 call sites: `0x1000f22d/0x1000f48a/0x1000f7b3`); `AutoNest.dll` imports only the global `Rmax_CircleFit` and writes it (5000.0) in the same dead `Dxf2Grps` path. **[VERIFIER] Not exercised by Mlaser's live nesting path** |
...
There is **no** DWG import in the normal CAD path (only DXF, PLT, G‑code/NC and the native `.chf`); DWG support exists only inside the nesting bridge via the ODA library — and [VERIFIER] even there it is unreachable from any exported entry point (§3.1), so the shipped product has no working DWG import at all. There is **no bitmap/raster import or engraving** anywhere (no `potrace`‑like strings, no image entity handling; GDI+ is used only to *write* JPEG thumbnails).
...
MainApp.exe (MFC100u, BCGCBPRO2210u100, OPENGL32, GLU32, gdiplus, HID, SETUPAPI, WINHTTP, …)
 └─ LoadLibrary "\Module\*.dll"   (UTF‑16 string @ MainApp.exe.utf16 line 5456)
     ...
     │         └─ (Dump/…, dat/tmpnst1 temp dirs — the "%s/tmpnst%d" pattern lives in SmartNest.dll, not AutoNest)
     ├─ Module/ControlModule.dll, LangModule.dll, LogModule.dll, NCModule.dll, ParaModule.dll (no geometry imports)
 MotionCtrl.dll (top-level, 2021‑07‑29, pdb "\MotionCtrl_dll_V1.3.22\Release\MotionCtrl.pdb")
     └─ imports splineAnalyerVc100.dll (newSpline2DAnalyer)   [VERIFIER addition — objdump -p MotionCtrl.dll]
        RTTI: CContoutSmooth, IContourSmooth, CVelocityPlanning, IVelocityPlanning, CNurbsContour (same class names as CADModule);
        no binary in the package contains the string "MotionCtrl" → not LoadLibrary'd either; dead file (see §8 q.9)
```
...
* Supported drawing versions (ASCII string table — [VERIFIER] not UTF‑16; `strings -a Dxf2Grp.dll | grep '^AC1'`): `AC1001 … AC1014, AC1015 (R2000), AC1018 (R2004), AC1021 (R2007)`, plus `AC1500` (ODA's internal "R2007 beta"). **No `AC1024` (R2010) or later** — consistent with a 2007‑vintage DWGdirect 2.3.
...
**INFERENCE (high):** `Dxf2Grp.dll` is the *SmartNest* vendor's part importer (same vendor as `AutoNest.dll`; the ".xjy" config and "SmartNestCAD" strings are theirs). Its role is to turn arbitrary DWG/DXF into closed loops of lines/arcs/circles (splines/ellipses discretised then arc‑fitted via `CircleFitDLL`) suitable for NestLib.

**[VERIFIER — corrected] Reachability in Mlaser (EVIDENCE, `objdump -d AutoNest.dll` + caller tracing):** `AutoNest.dll` reaches `Dxf2Grps` only through the import thunk `0x1006a420` (`jmp ds:0x1007501c`), whose single caller is `0x1000b6e7` inside function `0x1000b417`; that function's only caller is `0x10005f30`, and `0x10005f30` has **no caller and no data reference anywhere in the image** (no vtable / message‑map entry — a byte search for `30 5f 00 10` finds nothing). None of the exported `Nest_*` functions can reach it. The same function (`0x1000b417`) is the only place that writes `s_dblCompressLen` (= 1.0 from `.data 0x1007a580`) and `Rmax_CircleFit` (= 5000.0 from `0x1007a588`) before calling `Dxf2Grps`. Consequently, in the shipped Mlaser build **the CADModule → `Nest_WritePartData` path never uses `Dxf2Grp.dll`/ODA/`CircleFitDLL`**; the DLLs are loaded only because of static import binding. The analyst's "reached only when the user nests parts loaded from files" is refuted — there is no live caller at all (confidence high). The DWG import capability is therefore *present in the binary but unreachable* from Mlaser's UI.

**INFERENCE (high — upgraded by VERIFIER):** `SetPara(bool bFit, double dblFitTol, bool bExportNoClose, bool bAllRestToQG, double dblMinDia, bool bTipSameEnt, double dblCurveTol, double dblTol)` — the decorated export `?SetPara@CDrawing@@QAEX_NN00N0NN@Z` encodes exactly the type sequence `(bool, double, bool, bool, double, bool, double, double)` (`_N` = bool, `N` = double, `0` = back‑reference to the first type), so the type pattern is confirmed; the *names* per position remain the analyst's inference (medium).
...
**Identity (EVIDENCE):** VS_VERSION_INFO: FileDescription "HePin DLL", InternalName/OriginalFilename "HePin", LegalCopyright "版权所有 (C) 2014"; a dialog "用户授权" [User authorisation] with fields 用户编码 [user code], 原授权码 [old licence code], 新授权码 [new licence code] and the text "感谢您使用SmartNest智格套料软件 … 武汉智格科技有限公司 公司网址 www.smartnest.com.cn 服务邮箱 smartnest_service@163.com 联系电话 027-87770198,15827029286" [Thank you for using SmartNest nesting software … please check the system date … obtain a new authorisation code from Wuhan SmartNest Technology Co., Ltd.]. VC6/MFC42 build, PE timestamp 2023‑04‑28. **[VERIFIER addition]** PDB path (GBK, `.rdata` file offset 0x91000 after the `NB10` CodeView header): `D:\Code\奥森迪克DLL\HePin\Release\AutoNest.pdb` — "奥森迪克" (Àosēndíkè, a transliterated company/brand name, not SmartNest's) suggests this build of the bridge was produced for/by a third party (likely the SC2000 OEM) rather than at SmartNest; the older debug leftover `D:\FE\异形dll\HePin新\Debug\dat\test1.dxf` ("异形dll" = free‑form‑shape DLL, "HePin新" = new HePin) is a different tree.
...
| `Nest_SetPartPara(int sel, int value, int key)` | 3 | `switch(sel)` 0/1/2 → three setters on the global nest object `0x10089558`; each formats `key` with `"%d"` and stores `value` into a `CMap<CString,…>` at object offset `+0x84` (sel 0), `+0xa4` (sel 1, **stores constant 0**, `value` ignored) or `+0x94` (sel 2) (high — `0x100187c7/0x100188b7/0x1001883f`). **[VERIFIER]** CADModule calls it as `(partId, 1, 0)`, `(partId, 0, 1)`, `(partId, 1, 2)` with `partId = [part+0x34]` (the same value it passes as first arg of `Nest_WritePartData`), i.e. the *part id* lands in the `switch` selector: the calls are effective only for part ids 0, 1, 2 and are no‑ops for all other parts — an apparent API mismatch between CADModule and this AutoNest build (high for the mechanics, medium for "mismatch" vs. intended) |
| `Nest_WritePartData(int partId, int quantity, int nContours, contour* geom)` | 4 | **[VERIFIER — argument roles from both sides]** CADModule builds the 16‑byte arg block as `{[part+0x34], [part+0x38] (checked ≥ 1), nContours, ptr}` (`0x100d2925…0x100d2ad9`); AutoNest formats arg 1 with `CString::Format`, compares arg 3 with 0, and **clamps arg 2 to 200 (`0xc8`) when the dongle level `ds:0x1008814c` is 0/‑1 ("≤ 50") or to 5000 (`0x1388`) when the level is 1 ("≤ 100")** — two licence tiers of maximum part quantity (`0x1004caf3–0x1004cb37`). It then writes a `.grp` text file (string `.grp` at `0x1004cbd4`; error 7 "找不到grp" refers to it). Returns int status (0 = ok) |
...
**NestLib binding (EVIDENCE):** function `0x100534ab` does `LoadLibraryA("smartnest.dll")` then **98** × `GetProcAddress` ([VERIFIER] re‑counted: 98 `call ds:0x10075060` in the function; the analyst's "96" is wrong) storing the pointers at **`obj+0x08 … obj+0x18c`** of a *global* object at **`0x100897a8`** (`mov ecx,0x100897a8` before both calls of the binder at `0x1000e89d`/`0x1004a272`; the `HMODULE` sits at `obj+0x04`). Order of the `.data` name table (0x10088444…): `NOpenNestLib(+0x08), NStartSettingCallBacks, NSetErrorHandlerCB, NSetProgressFunctionCB, NSetNewPartGeomHandleCB, NSetNewSheetGeomHandleCB, NSetNestedSheetResultCB, NSetNestedOrderResultCB, NEndSettingCallBacks, NStartNewPartList, NStartSetParams, NEndSetParam, NAddPartInfo, NEndNewPartList, NStartNestOrder, NAddSheetInfo, NNestOrder, NSetStopNesting, NCloseNestLib, NSetApplyBestNestingDirection, NSetGuillotineCut, NSetGuillotineCutMinimumPhysicalDistance, NSetGuillotineCutSmallPartDistance, NSetGuillotineCutDistanceRatio, NDwgGeomStart … NDwgEndGetGeom (geometry feed, 22 names incl. NDwgClosedPunchProfStart/End, NDwgLeadinsStart/End, NDwgChangeCutterDia, NChgPartMinQty/MaxQty), NSetResolution/NInqResolution, NSetClearance/NInqClearance, NSetFitPartInPart, NSetEnableTurboMode, NSetConnectingDistance, NInqConnectingDistance, NInq* (results, 20 names), NSetFlatEndParams, NOffset* (11), NPart* (7), NInqNestedInstance, NStartGroup/NAddGroupMember/NEndGroup, NSetPartSeqMethod(+0x18c)`. Then `NOpenNestLib(<CString path>, 1, errorCB=0x1005424f)` (matches NestLib's own trace format `NOpenNestLib(%s, %d, %d);`), and the seven callbacks are registered (`0x10054168`, `call [obj+0x0c … +0x28]`).

**[VERIFIER addition] NestLib functions AutoNest actually calls** (EVIDENCE: every `call DWORD PTR ds:0x100897xx/0x100898xx/0x100899xx` resolved against the slot table; register‑relative calls checked for a preceding load of `0x100897a8`):

| Called (× sites) | Never called although resolved |
|---|---|
| `NOpenNestLib`, 7 callback setters, `NStartSetParams`(2), `NEndSetParam`(2), `NSetClearance`(1), `NSetResolution`(1), `NSetFitPartInPart`(2), `NSetEnableTurboMode`(1, conditional), `NSetConnectingDistance`(2, constant `0x3fb999999999999a` = **0.1**), `NSetApplyBestNestingDirection`(1), `NStartNewPartList`, `NAddPartInfo`(1), `NEndNewPartList`, `NStartNestOrder`, `NAddSheetInfo`(2), `NNestOrder`(1), `NDwgGeomStart`(2) / `NDwgProfsStart`(2) / `NDwgClosedProfStart`(3) / `NDwgAddLine`(4) / `NDwgAddArc`(8) / `NDwgAddCircle`(3) / `NDwgClosedProfEnd`(3) / `NDwgProfsEnd`(4) / `NDwgGeomEnd`(4), `NInqPartNested`, `NInqSheetTotalNested`, `NInqNestedPartType`, `NInqNestedSheet`, `NInqNestedInstance`, `NOffsetStart/ProfileStart/SingleProfile/ProfileEnd/InqTotalEntities/InqEntityType/InqLine/InqArc/End` (profile offsetting *is* used, 1–4 sites each) | **all four `NSetGuillotineCut*`**, `NSetStopNesting`, `NDwgStartGetGeom/EndGetGeom`, `NDwgPrimsStart/End`, **`NDwgPunchProfsStart/End`, `NDwgClosedPunchProfStart/End`** (holes are *not* fed as punch profiles), `NDwgLeadinsStart/End`, `NDwgChangeCutterDia`, `NChgPartMinQty/MaxQty`, `NInqResolution/NInqClearance`, `NSetFlatEndParams`, `NInqGetGuillotineCut`, `NInqGetBuildInfo`, most other `NInq*`, `NPart*`, `NStartGroup/NAddGroupMember/NEndGroup`, `NSetPartSeqMethod` |

Not even in the name table (so certainly unused): `NSetCutterDiameter`, `NDefineAngleList`, `NSetCollar*`, grain/mirror setters. Rotation step, sheet margin and direction are therefore *not* passed through dedicated API calls; the bridge formats them into text (`idx2` via `"%.f"` at `0x1000769c`, direction names `水平方向(从左往右)/竖直/自动(根据排样效果)` selected by `idx3 = 1/2/3` at `0x1000762b–0x1000767f`) — presumably the NestLib parameter/order file consumed between `NStartSetParams`/`NEndSetParam` (INFERENCE medium; the file writer was not decompiled).
...
**Licensing/dongle (EVIDENCE):** `HID#Vid_3689&Pid_8762`, registry `SYSTEM\ControlSet001\Control\DeviceClasses\{4d1e55b2-f16f-11cf-88cb-001111000030}`, dynamic `HidD_GetFeature/SetFeature/GetAttributes…`, `SetupDiGetClassDevsA…` (0x100890a0–0x100892b8); vendor string `PWDKeyCo.,` (0x10075e2c). Four 64‑hex‑digit constants at 0x10088f3c… match **four of the six** **SM2** (GB/T 32918) recommended curve parameters: `a = FFFFFFFEFFFF…FFFC` (0x10089008), `b = 28E9FA9E9D9F5E344D5A9E4BCF6509A7F39789F515AB8F92DDBCBD414D940E93` (0x10088fc4), `Gx = 32C4AE2C1F1981195F9904466A39C9948FE30BBFF2660BE1715A4589334C74C7` (0x10088f80), `Gy = BC3736A2F4F6779C59BDCEE36B692153D0A9877CC62A474002DF32E52139F0A0` (0x10088f3c) — [VERIFIER] the prime `p` and order `n` are **not** present as hex strings (`xxd 0x88f00–0x89060`), so either they are stored in binary form elsewhere or the code only does point arithmetic with these four; "SM2‑based" remains a medium inference. Nearby: hex digit table `0123456789ABCDEF`, `%02X`, `\\.\`. ~250 16‑hex‑digit tokens at 0x10080856… (INFERENCE medium: a licence/ID table). The same VID/PID is the "USBKey" dongle plugged into the source PC. Detailed dongle protocol belongs to the licensing analyst; what matters here is that **nesting is gated by this dongle** (`Nest_CheckLock` → error 1 "没找到加密狗" [dongle not found]).
...
**Identity (EVIDENCE):** PDB `\opennurbs_Dll\OpenNurbs_Dll_v1.3.14\Release\splineAnalyerVc100.pdb`; **123** distinct `.?AVON_*` RTTI names ([VERIFIER] re‑counted with `strings | grep -oE '\.\?AVON_[A-Za-z0-9_]+' | sort -u`; the analyst's "~180" is an over‑count) (`ON_NurbsCurve`, `ON_NurbsSurface`, `ON_Brep`, `ON_Mesh`, `ON_ArcCurve`, `ON_PolyCurve`, `ON_PolylineCurve`, `ON_BinaryArchive`, `ON_3dmObjectAttributes`, …), 26 embedded source‑file names `..\Nurbs\OpenNurbs\opennurbs_{3dm_attributes,3dm_properties,3dm_settings,arccurve,archive,brep,brep_io,brep_tools,curveonsurface,evaluate_nurbs,font,knot,layer,light,linecurve,math,mesh,nurbscurve,nurbssurface,nurbsvolume,object,objref,planesurface,polycurve,viewport,wstring}.cpp` and openNURBS diagnostic strings (`ON_BinaryArchive::BeginRead3dmChunk() - file is damaged.`, `ON_Font::Read - get newer version of opennurbs`, `Data size in 3dm archive: %d bytes`). This is a statically linked openNURBS (no `opennurbs_subd*`/`ON_SubD` → pre‑v6, i.e. Rhino 4/5‑era, INFERENCE medium); no copyright string of the wrapper author, and no Rhino/McNeel copyright string either.
...
**INFERENCE (high for the role, but see scope):** fits sequences of polyline points into arcs/lines with tolerance `Err`, minimum segment length `Len`, max included angle `Ang`, max radius `Rmax` — the "复杂曲线拟合压缩" [curve fit & compress] step of **Dxf2Grp's** DWG/DXF→part conversion. **[VERIFIER — downgraded]** `Dxf2Grp.dll` is the only caller of `mainFit` (3 sites) and the only writer of `Err/Len/Ang_CircleFit` (16 references); `AutoNest.dll` imports nothing but `Rmax_CircleFit`, which it writes (5000.0) solely in the unreachable `Dxf2Grps` path (§3.1). So the "fit before nesting" step exists in code but is **not executed in Mlaser's live nesting path**; the contours CADModule hands to `Nest_WritePartData` are whatever CADModule produced (line/arc contours from its own model — CADModule does not import CircleFit either). This also answers the analyst's open question 4 about where `Err/Len/Ang` are set: inside `Dxf2Grp.dll`, from its `[m_dblFitTol]`/`[m_dblCurveTol]`/`[m_dblTol]` settings (INFERENCE medium).
...
* **HPGL/PLT**: `CPltFileParse`, `CPltFileRead`, `CPltInputFile` are global‑namespace classes ([VERIFIER] RTTI `.?AVCPltFileParse@@`, not `PltParse::`); the entity model is in `namespace PltParse`: `CPltEntity`, `CPltSegment2d`, `CPltArc2d`, `CPltCircle2d`, `CPltEllipseArc2d`, `CPltRect2d`, `CPltNubrs2d`, `CPltFileParseException`. 2‑letter HPGL mnemonics present as NUL‑delimited tokens in `.rdata` ([VERIFIER] re‑extracted from `objdump -s -j .rdata`; `strings` with its default 4‑char minimum cannot see them): `AA AR AT BR BZ CI CO DF DT EA EP ER EW IN IP KL LB OD OE OH OI OP OS PA PD PE PG PM PR PU RA RR RT SC SM SP WG` — i.e. HPGL/2 incl. absolute/relative arcs (`AA/AR/AT`), rectangles (`EA/ER/RA/RR`), Bézier (`BR/BZ`), polyline‑encoded (`PE`), labels (`LB`), polygon mode (`PM/EP`), wedge/edge wedge (`WG/EW`), and the device‑query set (`OD/OE/OH/OI/OP/OS`, presumably recognised and ignored). The analyst's `SI` and `FT` are **not** present.
...
**Text (EVIDENCE):** `CGlyText`, `CTextGlyphAdapter/ITextGlyphAdapter`, `COpText2GroupCmd`, `CGlFont3D`, `CWglFontBitmap`; GDI imports `CreateFontA` (one call site `0x100d4478`; [VERIFIER re‑read of the 14 pushes] height=`ebx`, width/escapement/orientation 0, weight `0x1f4`=500=FW_MEDIUM, italic/underline/strikeout 0, charset from a register, out/clip precision and quality 0, pitch‑and‑family byte `0x20` = `DEFAULT_PITCH | FF_SWISS` — not a "pitch" value — and face name from `edx`), `SelectObject`, `GetGlyphOutlineW` (exactly two call sites, `0x100e6aa4` and `0x100e6aed`, both with `push 0x3` = `GGO_BEZIER` in the `uFormat` slot — first with `cbBuffer=0/lpvBuffer=0` for sizing, then with the buffer), `DeleteObject`; OpenGL `wglUseFontBitmapsW` (2 sites, `0x100d4502`, `0x100d46d2`), `glBitmap`, `glCallLists` for on‑screen labels. `wglUseFontOutlinesW` is *not* imported, so text geometry never comes from OpenGL.

**[VERIFIER addition] Debug trace files written by CADModule into the working directory (EVIDENCE: the file names are ASCII strings in `Module/CADModule.dll` and the files exist in `SRC` root):** `before_mergeLinearGly.txt`/`after_mergeLinearGly.txt`/`after_smoothGly.txt` (each "284.266 / total: 284.266" — a path length before/after collinear merge and smoothing), `after_setDataWithoutReFit.txt` and `setDataWithoutReFit_segs.txt` (segment counts/lengths of a spline `SetData` without re‑fit), `segments.txt` (24 line segments as `(x, y) (x, y)` pairs), `linkFlyLine_pathGlys.txt` (`2, 25 / 2, 3 …` — fly‑cut path glyph links), `closePwmPosRatios.txt` (46 ratios 0…1 — PWM close positions along contours), `p_micoLinkLenPos.txt` and `calcGraphCtInterpPt.txt` (empty). They name internal routines (`mergeLinearGly`, `smoothGly`, `setDataWithoutReFit`, `calcGraphCtInterpPt`, `linkFlyLine_pathGlys`, `micoLinkLenPos`) and show the release build still writes debug dumps on every job — the Linux port should not replicate this, but the names are useful anchors for the geometry pipeline (merge collinear → smooth → interpolate → link fly‑lines → micro‑joint positions → PWM close ratios). MainApp UI: `gp77 字体/Font`, `gp78 字号/Font NO`, `gp79 宋体/Segoe UI`, `ti0 文字高度(mm)/Font Height(mm)`, `mv20 请输入文字内容/Please input text`, `mv33 文字转曲线/Text Convert Spline`, `pd374 自动转换文字为曲线/Convert text to curves`, `mf251 精品字/Adv Text`, `mf254 广告字/Advert Text` (with `pd1810/1811` type & offset distance — INFERENCE: single‑stroke/offset "sign‑making" text), `font_MicrosoftYaHei`, `CTextInputDlg`, `CGlyphPropPanel`.
...
| DWG (R2.5–R2007) | import — **present in code but unreachable** ([VERIFIER] the only `Dxf2Grps` caller chain in AutoNest has no entry point, §3.1) | AutoNest → Dxf2Grp (ODA) | `AC1001…AC1021`, `Loading DWG file...`, `Dxf2Grps` import |
...
**[VERIFIER — mapping re‑derived from how AutoNest *consumes* each stored value; the analyst's original column is kept for the record.]** `Nest_SetNestPara(idx, double v)` first formats `v` with `"%0.4f"` (`0x100881c0`) and converts it back with `atoi` (`ds:0x1007545c`) into `[ebp-0x14]`; "int" slots below store that truncated integer, "double" slots store the raw double.

| idx | Source field in `CSCNest` | Stored at (AutoNest) | Type / post‑processing in AutoNest | Analyst's INFERRED meaning | **VERIFIER: observed consumer → corrected meaning (confidence)** |
|---|---|---|---|---|---|
| 0 | `[esi+0x48]` double | `0x10088108` | double | part spacing `pd_Nest_PartSpace` (also passed as `NSetClearance`) | Copied to `0x100893a8` (`0x100075a1`); **never passed to NestLib**. At `0x100075d6` it is compared with idx1: if idx0 < idx1, the sheet's two size globals `0x10089aa8/0x10089aa0` are enlarged by `2.0 × (idx1 − idx0)` (`fmul ds:0x10075670` = 2.0) and the results are shifted back later (`fsubr` at `0x10007e34/0x10007e4c`). That is the classic trick to obtain a *sheet edge margin smaller than the part clearance*: **idx0 = edge stock `pd_Nest_EdgeStock`** (medium‑high; refutes analyst) |
| 1 | `[esi+0x50]` double | `0x10088110` | double | edge stock `pd_Nest_EdgeStock` → COLLAR | Copied to `0x10088d48` (`0x1000758b`), **zeroed when idx6 ≠ 0** (`0x100075c2`), and `0x10088d48` is the argument of the single `NSetClearance` call (`0x1000e8c4`): **idx1 = part spacing `pd_Nest_PartSpace`** → NestLib CLEARANCE (high; refutes analyst). Zeroing it under idx6 only makes sense if idx6 is the common‑edge switch |
| 2 | `[esi+0x58]` double | `0x10088118` | **quantised to {90, 180, 270, 360}**: `v ≤ 90 → 90; ≤180 → 180; ≤270 → 270; else 360` (constants 0x10075bf0/0x10075710/0x10075be8 verified = 90.0/180.0/270.0; stores 0x40568000/0x40668000/0x4070e000/0x40768000 = 90/180/270/360) | rotation step angle | Confirmed. Consumed by `CString::Format("%.f", idx2)` into `0x100893b0` (`0x1000769c`) — written to NestLib's text parameters, not via an API call. **idx2 = `pd_Nest_PartRotateStepAngle`**, 90° multiples only (high) |
| 3 | `[esi+0x60]` int (fild) | `0x10088120` | int | nest direction (0 = L2R, 1 = D2U) | `switch` at `0x10007636`: **1 → "水平方向(从左往右)" [horizontal, left→right], 2 → "竖直" [vertical], 3 → "自动(根据排样效果)" [auto by result]**, other → none; codes are 1/2/3, not 0/1 (high for codes; the mapping of `pd_Nest_NestDir_L2R/D2U` onto 1/2 is medium) |
| 4 | `[esi+0x68]` double | `0x10088138` | double | min common‑edge length | Passed **directly to `NSetResolution(double)`** (`0x1000e8e7 → call ds:0x10089868` = slot +0xc0) and, in `Nest_WriteSheetData`, a computed segment length below it raises error 15 "线段太短" (`0x1004d467 → mov ds:0x10089418,0xf`). **idx4 = nesting accuracy `pd_Nest_NestAccuracy` → NestLib RESOLUTION** (high; refutes analyst) |
| 5 | computed bool: `[esi+0x70]` AND all selected parts closed/valid (`[ebp-0x59]`) | `0x10088148` | int | enable common edge | `cmp ds:0x10088148,0` selects **`NSetFitPartInPart(1,1)` vs `(0,0)`** (`0x1000e8f6–0x1000e918`, slot +0xd0). **idx5 = nest‑in‑holes `pd_Nest_NestPartInner`** (high; refutes analyst). The "all parts closed" precondition in CADModule fits: only closed parts can host others |
| 6 | `[esi+0x71]` byte | `0x10089774` | int | nest in part inner | Checked at `0x100075b9` (zeroes the clearance), `0x100077e7`, and `0x1000f26e` where it gates a large block (`0x1000f27b` allocates a 0xc8‑byte object … up to `0x100104f1`) that uses the `NOffset*` profile‑offset API. **idx6 = enable common edge `pd_Nest_ShareEdge`** (medium‑high; refutes analyst) |
| 7 | `[esi+0x78]` double | `0x10089780` | int → double | accuracy / RESOLUTION | Used only inside the idx6‑gated block: `fcomp ds:0x10089780` at `0x1000f553` against a per‑object virtual getter (`call [eax+0x48]`, plausibly an edge length); when smaller, a candidate flag `[ebp-0x88]` is cleared. **idx7 = min common‑edge length `pd_Nest_ShareEdgeMinLen`** (medium; refutes analyst) |
| 8 | (not set by CADModule) | `0x10089778` | int | reserved / only‑selected? | Checked at `0x100078f4` and `0x10008407` in the core; when non‑zero, `0x1001892f` is called with `push 0`. Meaning still unknown; not set by Mlaser (low) |

Net effect for the port: the *pairs* (0,1) and (4,5,6,7) were swapped by the analyst; the values Mlaser's UI exposes (spacing, edge margin, step angle, direction, nest‑in‑holes, common edge + min length, accuracy) are all still the ones that reach the kernel, but the accuracy value is NestLib's *resolution* (a length, also the minimum accepted segment length), and the sheet‑margin is emulated by enlarging the sheet, not by a NestLib collar.

Sheet: `Nest_WriteSheetData({idx, isRect(byte), 1, W, H, …})` from `[edi+0x44]` (rect flag), `[edi+0x48]`/`[edi+0x50]` (width/height); non‑rectangular sheets pass contour arrays (`mf462` import sheet from drawing; `A250212_5 自动识别板材尺寸` auto‑detect sheet size by capacitive edge finding is a separate machine function). Parts: `Nest_WritePartData(partId = [part+0x34], quantity = [part+0x38], nContours, contours*)` ([VERIFIER] roles from the CADModule arg‑block build at `0x100d2925…0x100d2ad9` and AutoNest's use of each slot, §3.3; quantity is clamped by AutoNest to 200 or 5000 depending on dongle tier) then `Nest_SetPartPara(partId,1,0)`, `(partId,0,1)`, `(partId,1,2)` — [VERIFIER] because AutoNest switches on its *first* argument, these are effective only for `partId ∈ {0,1,2}` (§3.3); the analyst's "quantity / priority / rotation flags" reading is unsupported. Progress callback `0x100cc6b0` registered via `Nest_RegProgressCB`. Results: `Nest_ReadNestNum()` (1…20000 sheets — `lea ecx,[eax-1]; cmp ecx,0x4e1f; ja` at `0x100d3637–0x100d3646`, verified), per sheet `Nest_ReadPartNumbyNest(i)` → allocate `n×48` bytes → `Nest_ReadNestResult(&buf, i)` (`imul edx,edx,0x30` in AutoNest verified; the result header stores `nParts` at `+0x10` and a second int at `+0x14`, and copies per‑element dwords at `+0x10/+0x14/+0x18…` of a source record — layout of the five doubles still unverified).
...
* **True‑shape**: the kernel is NestLib (polygon‑based, raster resolution); geometry fed as LINE/ARC/CIRCLE closed profiles (`NDwgGeomStart/NDwgProfsStart/NDwgClosedProfStart/NDwgAddLine/NDwgAddArc/NDwgAddCircle/NDwgClosedProfEnd/NDwgProfsEnd/NDwgGeomEnd` — all verified call sites, §3.3); part‑in‑part via `NSetFitPartInPart` (verified, driven by idx5). **[VERIFIER — corrected]** Holes are **not** fed as punch profiles: `NDwgPunchProfsStart/NDwgClosedPunchProfStart` are resolved but never called; inner contours must therefore be sent as additional closed profiles (NestLib infers holes from containment — INFERENCE medium).
* **Rotation**: NestLib supports arbitrary angle lists, but the shipped bridge quantises the step to 90/180/270/360 (§5.2) and never resolves `NDefineAngleList`.
* **Rectangular/guillotine**: **[VERIFIER — refuted]** `NSetGuillotineCut*` are resolved into the table but **never called**: no `call ds:0x100897f8…0x10089804` exists, and none of the register‑relative `call [reg+0x58/0x5c/0x60/0x64]` sites has the NestLib object base loaded nearby (they belong to other C++ objects). The three disassembly lines the analyst cited (134073, 77084, 23190) are plain `mov ecx,…` instructions. The "矩形板材"/"Simple Nest" mode therefore does not use NestLib's guillotine engine; it is either the same true‑shape run with different parameters or done in CADModule (unknown).
* **Sheets/remnants**: multiple sheets per order (`NAddSheetInfo` ×2 verified call sites, not 4), `sheet_used.txt`, "2nd nest" special sheet/part files, `Nest_GetOddCountByNest/Nest_ReadOddData` (余料 remnants), UI `残板` [remnant sheet] names/counts. Remnant *shape* generation appears to be done by CADModule (`CCutupScrap`) rather than NestLib's MASTER_PLATE.
* **Spacing**: single global part‑to‑part clearance (`NSetClearance(idx1)`) and a sheet edge margin emulated by enlarging the virtual sheet by `2·(spacing − margin)` when margin < spacing (§5.2); `NSetConnectingDistance(0.1)` is hard‑coded; kerf is handled separately by CADModule's compensation, not by NestLib's CUTTERDIA (`NSetCutterDiameter` is not even in the name table — verified).
* **Profile offsetting**: AutoNest does call NestLib's `NOffset*` API (§3.3), inside the idx6 (common‑edge) block — INFERENCE (medium): used to build common‑edge candidates / offset profiles for shared cuts, which is then post‑processed by CADModule's `CShareEdge`.
...
2. **`Nest_SetNestPara` index 8 and `Nest_SetPartPara`.** CADModule never sets idx 8 (AutoNest tests it at `0x100078f4/0x10008407`). [VERIFIER] `Nest_SetPartPara` is now understood mechanically (three `"%d"`‑keyed maps at nest‑object `+0x84/+0xa4/+0x94`, selector = first argument) and CADModule's calls only take effect for part ids 0–2 — what the three maps *mean* to the `.grp`/part‑list writer, and whether the mismatch is a bug, remains open.
...
4. **How CADModule hands part geometry to AutoNest** (`Nest_WritePartData(partId, quantity, nContours, contours*)`): the contour record format behind `contours*` is still unknown (line/arc records vs. polylines). [VERIFIER] The CircleFit question is closed: `Err/Len/Ang` are written only inside `Dxf2Grp.dll`, `Rmax` only in AutoNest's unreachable `Dxf2Grps` path — CircleFit is not on Mlaser's live path.
5. **Which NestLib options the bridge enables** — [VERIFIER] resolved by the slot‑table call scan in §3.3: `NSetClearance`, `NSetResolution`, `NSetFitPartInPart`, `NSetEnableTurboMode` (conditional), `NSetConnectingDistance(0.1)`, `NSetApplyBestNestingDirection`, part list / sheet list / `NNestOrder`, closed‑profile geometry feed, `NOffset*` utilities and a handful of `NInq*`. **Not** used: guillotine, punch profiles, lead‑ins, part min/max quantity, groups, sequencing method, angle list. Still open: the exact text of the NestLib parameter/order file written between `NStartSetParams`/`NEndSetParam` (where step angle and direction go) and whether mirroring is enabled there.
...
9. **`MotionCtrl.dll`** (exports `arcInterp/newContourSmooth/newVelocityPlanning/segInterp*`) is not imported by MainApp or any Module DLL. [VERIFIER] Largely resolved: it imports `splineAnalyerVc100.dll` (`newSpline2DAnalyer`), its RTTI (`CContoutSmooth`, `IContourSmooth`, `CVelocityPlanning`, `IVelocityPlanning`, `CNurbsContour`) and the version string `1.3.23` duplicate what is statically inside CADModule, its pdb is `\MotionCtrl_dll_V1.3.22\Release\MotionCtrl.pdb`, and **no shipped binary contains the string "MotionCtrl"** (ASCII or UTF‑16), so it is not `LoadLibrary`'d by name either → a stale stand‑alone build of the same in‑house smoothing/velocity classes, dead in this package (high). Only a dynamic `LoadLibrary` with a constructed name could contradict this.
...
* Nesting UX and parameter set: spacing (= kernel clearance), edge margin (emulated in the bridge by enlarging the sheet — a native nester can apply it directly), rotation step (note the shipped kernel effectively uses 90° steps), direction L2R/D2U (bridge codes 1/2/3), nest‑in‑holes (kernel part‑in‑part), common‑edge enable + min length (bridge‑side post‑processing with zero clearance, not a kernel feature), accuracy (= kernel resolution and minimum segment length), standard sheet size, part quantities (bridge caps 200/5000 per dongle tier — drop), multi‑sheet results, remnant sheets, ≤ 50 part types, and the error taxonomy of §5.4 (map to equivalent messages). [VERIFIER] No guillotine/rectangular kernel mode needs to be replicated — it is never invoked (§5.3).
...
Adversarial re‑check of this document against the primary files under `SRC`, performed with `objdump -p/-d/-s`, `strings -a [-e l] [-t x]`, `xxd`, `iconv`, and three small stdlib‑Python helpers (PE export walker, caller‑chain tracer over the objdump output, slot‑table resolver). Every command was re‑run; nothing was taken from the analyst's scratch files except as a starting point for addresses.

### Confirmed as stated (evidence re‑observed)

* `Dxf2Grp.dll`: `DWGdirect 2.3.1.0`, `Copyright 2003-2007, Open Design Alliance Inc.`, `Open Design Alliance ACIS Builder` (ACIS 4.00/7.00 stamps), `Internal error in Freetype font library`, version list `AC1001…AC1021` + `AC1500`, no `AC1024`; imports only Win32/MFC42/CRT/`CircleFitDLL` (no `TD_*.dll`); VS_VERSION_INFO `Dxf2Grp DLL` / `版权所有 (C) 2009`; persisted `[m_*]` keys, `\smartnest.xjy`, dialog strings; PE timestamp 2015‑09‑21.
* `SmartNest.dll`: `Geometric`, `Nestlib Development Version`, `/mna/usr/home/nestlib/tbom`, `Can not find the hardware lock…`, `VERSION : 3.0, DATE : 30 JUN 95`, `20/09/2002`; 226 exports; imports USER32/KERNEL32 only; PE timestamp 2002‑10‑21.
* `AutoNest.dll`: `HePin DLL`, `版权所有 (C) 2014`, `武汉智格科技有限公司 … www.smartnest.com.cn`, `smartnest.dll` at `.data 0x10088434`, `HID#Vid_3689&Pid_8762`, `HidD_*`, `\hid.dll`, `\SetupApi.dll`, `PWDKeyCo.,`, 26 exports, `LoadLibraryA` + `GetProcAddress` loop at `0x100534ab`, NestLib text formats `LINE/ARC/CIRCLE %lf…`, temp file names, `nest_output_sheet.%d`, `Nest_SetNestPara` switch with 90/180/270/360 quantisation (constants verified as doubles), arities (`ret 0x10/0x28/0x8/0x4/0xc`), `n×0x30` result elements.
* Import graph: `MainApp.exe` imports none of the geometry DLLs and holds `\Module\*.dll`; `CADModule.dll` imports exactly `newDxfFileParse`, `?newSpline2DAnalyer@@…`, and the 18 `HepinDLL`/`Nest_*` symbols; `AutoNest` ← `Dxf2Grps`, `CPart2` dtor/`Release`, `CDPoint2` dtor, `s_dblCompressLen`, `Rmax_CircleFit`; `Dxf2Grp` ← 11 CircleFit symbols incl. `mainFit`, `Err/Len/Ang_CircleFit`.
* `splineAnalyerVc100.dll`: pdb `OpenNurbs_Dll_v1.3.14`, two exports, `push 0x6c; call malloc; jmp ctor` stubs, 25‑slot vtable at `0x100e0bb4` with slot1/2 `ret 0x10`, slot10 `ret 0xc`, slot20 `ret 0x30`.
* Formats: `mf149` filter string, `cp5`, `mf154`, `mv31`, `pd369/369_1/370/373/374`, `pd93`, `pd1810/1811`, `mf251/254`, `mf497–500`, `mf523`; G‑code BNF and error strings; `scFlie`, `<Begin Graphs>`, `<Glyphs>`, `<Crafts>`; `dxfTemplate.dxf` referenced by DxfParse and absent from `SRC`; no AI/SVG/PDF/EPS/LXD/potrace strings in MainApp/CADModule/DxfParse (only splash/help/logo `.bmp` and thumbnail `.jpg`); MainApp GDI+ imports limited to bitmap creation, JPEG encoders, `GdipDrawLineI`/pen/smoothing.
* Text: `GetGlyphOutlineW` at exactly two sites with `push 0x3` in the `uFormat` position; `CreateFontA` at one site with weight 500; `wglUseFontBitmapsW` at two sites; `wglUseFontOutlinesW` not imported; OpenGL 1.x immediate‑mode imports in both binaries.
* CADModule `Nest_SetNestPara` pushes idx 0,1,2,3,4,6,7 then 5 with `fld [esi+0x48/0x50/0x58]`, `fild [esi+0x60]`, `fld [esi+0x68]`, `movzx [esi+0x71]`, `fld [esi+0x78]`; `Nest_CheckLock()==0 → eax=0x97`; `Nest_ReadNestNum` range check `1…0x4e20`.
* `排样内核错误代码.txt` is UTF‑8 (BOM‑less, CRLF) with codes 0–15 and 100 exactly as tabulated.
* RTTI names in CADModule: all listed classes exist except the one corrected below.

### Refuted or corrected

1. **Dxf2Grp/ODA/CircleFit are not on any live path.** The only `Dxf2Grps` caller chain (`0x1000b6e7` in `0x1000b417` ← `0x10005f30`) has no callers and no data references; no exported `Nest_*` function reaches it. The document's "reached only when the user nests parts loaded from files" was wrong: it is unreachable (§1, §3.1, §3.6). Consequently CircleFit (called only from Dxf2Grp) is never executed by Mlaser.
2. **`Nest_SetNestPara` index mapping** (key fact 9, "likely"): re‑derived from consumers — idx0 = edge margin, idx1 = part spacing (`NSetClearance`), idx4 = accuracy (`NSetResolution`), idx5 = nest‑in‑holes (`NSetFitPartInPart`), idx6 = common‑edge enable (zeroes clearance, gates the `NOffset*` block), idx7 = min common‑edge length (used only inside that block); idx3 codes are 1/2/3. Four of the analyst's eight assignments were swapped (§5.2).
3. **Guillotine mode**: `NSetGuillotineCut*` are never called; the cited "call sites" were `mov` instructions (§5.3). Holes are not fed as punch profiles.
4. **`GetProcAddress` count** is 98, not 96; the pointer table is a *global* object at `0x100897a8` (`+0x08…+0x18c`), not `this`.
5. **"NestLib 3.x"**: the 1995 `VERSION : 3.0` stamp belongs to an internal component; the NestLib release version is unknown.
6. **`ON_*` RTTI count** is 123, not ~180; openNURBS is pre‑v6 (no SubD).
7. **Class names**: `CEditCmdArrayCmd` (not `COpEditCmdArrayCmd`); `CPltFileParse`/`CPltFileRead`/`CPltInputFile` are global, only the PLT entities live in `PltParse`.
8. **HPGL mnemonics**: `SI`/`FT` absent; `CO EW KL OD OE OH OI OP OS RT SM WG` present (extracted from `.rdata` bytes, since `strings` cannot show 2‑byte tokens).
9. **`CreateFontA`** `0x20` is the pitch‑and‑family byte (`FF_SWISS`), not a pitch value.
10. **SM2**: only `a`, `b`, `Gx`, `Gy` appear as hex strings; `p` and `n` do not — "SM2" stays a medium inference.
11. **Version table** of Dxf2Grp is ASCII, not UTF‑16; `NAddSheetInfo` has 2 call sites, not 4.
12. **`Nest_SetPartPara`** semantics: selector is the *first* argument; CADModule passes the part id there, so the three calls are no‑ops except for part ids 0–2 (§3.3, §5.2). The analyst's "quantity/priority/rotation flags" reading is unsupported.
13. **`Nest_WritePartData`** argument roles established: `(partId, quantity, nContours, contours*)`, with quantity capped at 200/5000 by dongle tier (§3.3).

### Added (missed by the analyst)

* `MotionCtrl.dll` imports `splineAnalyerVc100.dll`, duplicates CADModule's smoothing/velocity classes (pdb `MotionCtrl_dll_V1.3.22`), and is referenced by no binary → dead file (§2, §8 q.9).
* AutoNest pdb path `D:\Code\奥森迪克DLL\HePin\Release\AutoNest.pdb` (§3.3).
* Full list of NestLib API functions actually called vs. merely resolved (§3.3), incl. `NSetConnectingDistance(0.1)` and the `NOffset*` usage.
* Sheet‑enlargement trick for edge margin; clearance zeroed under common‑edge (§5.2).
* Licence tiers (`ds:0x1008814c` = 0/1 → 200/5000 parts per type) in `Nest_WritePartData`; `.grp` intermediate file (§3.3).
* `Nest_SetNestPara` converts the double through `"%0.4f"` + `atoi` for the integer slots (§5.2).
* CADModule writes ten debug dump files into the working directory on every job (`after_smoothGly.txt`, `segments.txt`, `closePwmPosRatios.txt`, …), naming internal pipeline stages (§3.7).
* `"%s/tmpnst%d"` (the `dat/tmpnst1` directory) is created by `SmartNest.dll`, not AutoNest (§2).
* `SetPara` type sequence confirmed from the decorated export name (§3.1).

### Still uncertain

* The exact per‑position names of `SetPara` arguments; the contour record format behind `Nest_WritePartData`'s pointer; the five doubles of the 48‑byte result element; idx8; the NestLib parameter/order text (where step angle, direction and mirroring are set); the meaning of the three `Nest_SetPartPara` maps and whether the id/selector mismatch is intentional; `ISpline2DAnalyer` slot semantics beyond slots 0–2/10/20 (the CADModule‑side `(…,0,3)` call pattern for slot 1 was *not* re‑verified here); PLT scale/units handling; "精品字/广告字" generation; `.chf` v4/v5 differences.
