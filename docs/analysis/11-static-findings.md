...
Tick period = `AX.InterpolationCycle` µs as refreshed from **reg 50005** (XML default 250). A "hold t ms" is `(1000 idiv cycle)·t` stationary ticks **only on the fibre pierce-stage path**; for a CO2 cut it is a card-side `2001[t, 3000]` wait record (§5.5, A9 §2).
...
Prologue: `3001; 3002[5]; tick(0,0); 3001; 9999[2,4,4] (DO3 high air on); 103[1000,0]; 2001[0x03000002,20000]; 9999[2,0x100,0x100] (DO9 CO2 laser enable on); N×tick(0,0,5000 Hz,4 %)`. The `N` stationary ticks are **not** a dwell — they are the cut ramp's own zero-displacement samples (§5.5).
Epilogue: `3001; 9999[2,0x100,0] (DO9 off); 109[1000,0]; 2001[0x03000002,20000]; 118[4,0,35]; 3001; 3002[4]`, then rapid ticks at 0 %.

### 5.5 Contour entry velocity and dwells (A9; closes X1 and X2)
**Dwells are records, not ticks.** The dwell builder `MainApp 0x4427e0` has a tick branch (`(1000 idiv AX.InterpolationCycle)·ms` stationary records) and a record branch (one type-1 sub-5 record → `2001[ms, 3000]`, dropped when `ms ≤ 0`). Nine of its ten call sites push `false`; the tenth (`0x442cf0`) is inside the multi-stage pierce emitter `0x442890`, which is entered only for a layer with `ManuType ≠ 0` (a fibre field) and which left no trace in the 22 leaked CO2 frames. The CO2 dwells are `layer+0x88` = `LaserOnDelay` (after the laser DO, `0x443259`), `layer+0x8c` = `LaserOffBeforeDelay` and `layer+0x90` = `LaserOffAfterDelay`. The **gas** delay is a separate builder, `0x4424e0`: it sums `GC.GasDelay` / `DirectGasDelay` / `ChangeGasDelay` per state flag, subtracts the travel time already spent and emits the remainder — again as a wait record. With `LaserOnDelay = 0` and the gas already flowing, the CO2 prologue emits nothing between `DO9 on` and the first tick, which is exactly what frames 0x39/0x1f8 show (all three `2001` in the 22 leaked frames are the ZF `[0x03000002, 20000]` form). **Correction to §5.3 and to 04/00**: `GC.GasDelay` is not a tick dwell, and `LaserOnDelay + GasDelay` is not a sum.

**Every contour starts and ends at rest.** CADModule owns its own copy of `CVelocityPlanning` (`plan` `0x100fffe0`, node builder A `0x100ff220`, slow start `0x100ffac0`, look-ahead core `0x100fdc50`); the `0x1001xxxx` addresses of 05 §7 belong to `MotionCtrl.dll`'s build and are not what a cut runs. Node builder A finishes with `node[last].v = 0` and `node[0].v = 0` (`0x100ff8dd–0x100ff90c`). The slow-start routine that runs after it only writes `min(field, P7)` — a ceiling over the first `P6` mm, never a floor; the only floor, `P4`, is a constant 0 and is applied before the zeroing; and the look-ahead core takes just `{A, J, Ta, P5, Vmax}` with no entry-velocity argument. `Ta` is clamped to `[0.06, 0.25]` and `J = (A·Ta/2 ≤ Vmax) ? 2A/Ta : 4·Vmax/Ta²` in the build that runs (`0x1010000d`, `0x1010009c`). **So there is no "slow start" or minimum-velocity parameter that could make the laser leave the pierce point already moving.**

**The 90-pulses-in-113-ticks number was an artefact.** It came from reading frame `0x3a` as the continuation of frame `0x39`. The log shows `0x39` at 14:22:17 and `0x3a` at 15:04:19 with two job restarts between them (the frame id is the card's counter and restarts per job, A3 §0), so they are frames 57 and 58 of *different* runs. `0x3a` has four pulses in its first nine ticks (≈ 7 mm/s, a ≈ 400–580 mm/s²), which no start-from-rest profile at this machine's `J = 4·Vmax/Ta²` can produce; it is instead ≈ 145 ticks into a jerk-limited ramp, and the port's own S-curve reproduces its 99 ticks to within one pulse with `J ≈ 11 660`. What frames 0x39/0x1f8 do prove on their own is a **lower bound of 13** stationary ticks after `DO9 on` — which the port (≥ 66 for layer 2) satisfies. **Correction to 05 §7.2**: the `0.99` constant (`0x101116d0`, used at `0x100ff866`) is a `VelDecc.txt` logging threshold, not a factor on node speeds.
